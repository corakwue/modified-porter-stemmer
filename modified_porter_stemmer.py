...
Later additions:
   ...
   Several improvements added by Chuk Orakwue. For instance,
   now has 6 steps as step 1ab is repeated to handle corner cases for words
   ending with '-ingly', 'edly' etc.
...
class ModifiedPorterStemmer(StemmerI):
    ...
    def _consec_cons(self, word):
        """_consec_cons(word) is TRUE <=> word contain consecutive consonant"""
        ret = False
        for i in range(1, len(word)-1):
            if not self._vowelinstem(word[i-1:i+1]):
                ret = True
        return ret
        ...
        if ed_or_ing_trimmed:
            if word.endswith("at") or word.endswith("bl") or word.endswith("iz"):
                ...
                if word[-1] not in ['l', 's', 'z']:
                    word = word[:-1]
            #--DEPARTURE--
            elif (self._m(word, len(word)-1)%2 == 1 and self._cvc(word, len(word)-1)):
                word += 'e'
    ...
    def _step1c(self, word):
        ...
        if word[-1] == 'y' and len(word) > 2 and self._cons(word, len(word) - 2):
            if word.endswith("ology"): # --NEW--
                return word
            elif word.endswith("phy"): # --NEW--
                return word
            else:
                return word[:-1] + 'i'
        else:
    ...
    def _step2(self, word):
        ...
        if ch == 'a':
            ...
            # To match the published algorithm, replace "bli" with "abli" and "ble" with "able"
                ...
                return word[:-2] if self._m(word, len(word)-6) else word
            elif word.endswith("gli"):
                return word[:-2] if self._m(word, len(word)-4) else word # --NEW--
            else:
                return word
        elif ch == 'o':
            ...
            elif word.endswith("biliti"): # --DEPARTURE--
                return word[:-6] + "ble" if self._m(word, len(word)-7) or not len(word[:-7]) else word
            elif word.endswith("iti"): # --DEPARTURE-- # --NEW--
                return word[:-3] + "ity" if self._m(word, len(word)-4) else word
            elif word.endswith("uti"): # --DEPARTURE-- # --NEW--
                return word[:-3] + "uty" if self._m(word, len(word)-4) else word
            else:
                return word
        elif ch == 'g': # --DEPARTURE--
            ...
            # To match the published algorithm, pass len(word)-5 to _m instead of len(word)-4
                ...
                return word
        elif ch == 'r': # --DEPARTURE--, # --NEW--
            if word.endswith("ari"):
                # fixes premature translation of 'ary' --> 'ari' in step1c
                # Example: 'planetary' -> 'planet'
                return word[:-3] if self._m(word, len(word)-4) else word
            elif word.endswith("ori"):
                return word[:-3] + "e" if self._m(word, len(word)-4) else word
            elif word.endswith("tri"):
                return word[:-3] + "try" if self._m(word, len(word)-4) else word
            else:
                return word
        else:
            return word
    ...
    def _step3(self, word):
        ...
        if ch == 'e':
            ...
            if word.endswith("ness"): # --DEPARTURE--
                return word[:-4] if 2*(self._m(word, len(word)-5) - 0.5) > 1 else word
            else:
                return word
    ...
    def _step4(self, word):
        ...
        if ch == 'a':
            ...
            if word.endswith("ate"): # --DEPARTURE----
                return word[:-3] if self._m(word, len(word)-4) > 3 else word
    ...
    def _step5(self, word):
        ...
        if word[-1] == 'e':
            ...
            # --DEPARTURE--
            if (a == 1 and not self._cvc(word, len(word)-2) and not self._consec_cons(word)): #a > 1 or
                word = word[:-1]
        if word.endswith('ll') and self._m(word, len(word)-1) > 1:
            word = word[:-1]
        # --DEPARTURE--
        # --NEW--
        # CORAKWUE: I don't like this.
        # In some cases will return non-words.
        # Usually in those cases, len(stem) > len(word), we fix in self._adjust_case
        if word[-1] == 'r':
            if (len(word) > 3) and not self._consec_cons(word):
                word += 'e'

        if word[-1] == 'u' and not self._cvc(word, len(word)-1):
            word += 'e'
    ...
    def stem_word(self, p, i=0, j=None):
        ...
        if self.verbose:
            print('Step1ab: {}'.format(word))
        word = self._step1c(word)
        if self.verbose:
            print('Step1c: {}'.format(word))
        word = self._step2(word)
        if self.verbose:
            print('Step2: {}'.format(word))
        word = self._step1ab(word)
        if self.verbose:
            print('Step6(1ab): {}'.format(word))
        word = self._step3(word)
        if self.verbose:
            print('Step3: {}'.format(word))
        word = self._step4(word)
        if self.verbose:
            print('Step4: {}'.format(word))
        word = self._step5(word)
        if self.verbose:
            print('Step5: {}'.format(word))
